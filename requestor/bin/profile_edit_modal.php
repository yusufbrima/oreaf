  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <?php
        if(requestor::checkuser($userid,$conn->conn)==false){
          //$
          $result_set = requestor::retrieve_profile_display($userid,$conn->conn);
          ...
          <select class="sel" name="district">
            ...
            if($conn->checkQuery($query)){
              ...
              for($i=0;$i<$count;$i++){
                ...
                $id = $row['id'];
                $name =$row['name'];
                $selected = $result_set['districtID'];
                if($update_flag==true AND $selected== $id){
                   echo  "<option value='{$selected}' selected>{$name}</option><br />";
                }else{
                   echo  "<option value='{$id}'>{$name}</option><br />";
                          ...
                          if($validate->validate($_REQUEST['city'])){
                              if($validate->validate($_REQUEST['district'])){
                                 if($validate->validate($_REQUEST['region'])){
                                  if(requestor::checkuser($userid,$conn->conn)){ //Code block to perform insertion
                                      ...
                                      $middleName = isset($_REQUEST['middleName'])?$validate->sanitize(trim($_REQUEST['middleName'])):"";
                                       if(isset($_FILES)){ //start of file upload
                                            $result = (requestor::uploadImage($_FILES));
                                            if(is_array($result)){
                                              $photoSignature = $result[1];
                                              if(requestor::add_profile($firstName,$middleName,$lastName,$tel,$address,$userid,$photoSignature,$region,$district,$city,$conn->conn)){
                                     ...
                                     $tel = $validate->transform($validate->sanitize(trim($_REQUEST['tel'])));
                                      ...
                                      $lastName = $validate->sanitize(trim($_REQUEST['lastName']));
                                      $middleName = isset($_REQUEST['middleName'])?$validate->sanitize(trim($_REQUEST['middleName'])):"";
                                      if(requestor::update_profile($firstName,$middleName,$lastName,$tel,$address,$region,$district,$city,$userid,$conn->conn)){
