...
<!-- Collect the nav links, forms, and other content for toggling -->
	...
	<!--banner-->
		<div class="banner1">
		<div class="container">
			<h3 class="animated wow slideInLeft" data-wow-delay=".5s"><a href="index">Home</a> / <span>Request Pane</span></h3>
		</div>
	</div>
	<!--banner-->
	<!--Template body-->
  <div class="login-grids">
    <div class="login">
      ...
      <table class="table table-striped">
         <caption class="h1">Sent Requests</caption>
         <tbody>
	         <tr><th>Transaction ID</th><th>Posted By</th><th>Sent Date</th></tr>
	         <?php
						$query =  "SELECT t.railtor_id AS railtorid,t.logged_date AS loggeddate,t.id AS transactionid,t.requestor_id AS requestorid,t.property_id AS propertyid,CONCAT(r.firstName,' ',r.middleName,' ',r.lastName) AS fullname FROM transaction AS t LEFT JOIN railtor AS r ON t.railtor_id=r.id WHERE t.requestor_id='$requestor_id' AND trashed  =1";
						if($conn->checkQuery($query)){
							...
							for($i=0;$i<$count;$i++){
							    ...
							    $date_output = strtotime($row['loggeddate']);
      							...
      							echo "<tr><td>$t_id</td><td>$fullname</td><td>$date_output <a href=\"property_view?p_id=$p_id&r_id=$railtor_id&s_token=$hash_code\"><span class=\"glyphicon glyphicon-eye-open\"><!-- View -->  <a href='#' data-href='request?t_id=$t_id&r_id=$requestor_id&t_token=$hash_code' data-toggle='modal' data-target='#requestdelete'><span class='glyphicon glyphicon-trash'></span></a><!-- Delete --></span></a></td></tr>";
      						}
						}
				  ?>
       </tbody>
...
<?php
	...
	include_once("bin/request_delete_modal.php");
